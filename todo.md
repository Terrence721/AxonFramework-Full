...
| `common` module | `build.gradle.kts` skeleton done. Source conversion in progress: 82 files done, byte-identical to the Maven source (aside from the repo-wide `@author`-removal policy, see below), converted one at a time in dependency order. All 30 top-level files in `org.axonframework.common` are done, plus the `annotation`, `caching`, `digest`, `function`, `io`, `jpa`, `tx`, `nullability`, and `util` subpackages in full, `lifecycle` (4 of 5 — `Phase.java` deferred until `configuration` exists), `infra` (3 of 6, same `configuration`-dependency blocker for the rest), and `lock` (7 of 8 — only `package-info.java` left). 62 files under `src/main/java` still to go (plus 1 of 2 resource files), one at a time |
...
1. ~~**Build-logic convention plugins**~~ — done: `axonframework.java-conventions`, `axonframework.published-conventions`,
   ...
   stands.)
2. **Leaf modules** — `test-logging` done, `common` skeleton done and source conversion underway (82 of 144
