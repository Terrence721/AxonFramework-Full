...
8. ~~**CI workflows**~~ — done, scoped minimal: `.github/workflows/build.yml` runs `gradle build` on push/PR via
   `gradle/actions/setup-gradle`, with an explicit `permissions: contents: read` block. No dedicated quality-badge
   workflow yet (unlike this user's other portfolio forks) — there's barely any actual code for those to analyze
   until later phases land. GitHub's own default CodeQL setup runs regardless (not something configured here),
   found zero code issues so far, and already caught the missing-permissions gap above
