...
wholesale/automated conversion or a bulk copy of the source tree. **As of 2026-09-07, that decision can also be a
real redesign, not just a format conversion or bug fix** — where a genuine architectural improvement is found (and
verified safe against the real upstream Axon 5 source tree first, not just assumed), it's now in scope alongside
the targeted nullness/exception/typo fixes this project has always made. See the `configuration.Modules`/
`DecoratorDefinitions`/`ConfigurationEnhancers`/`ConfigurableComponentRegistry` row below for the first instance
of this. See the [project board](https://github.com/users/Terrence721/projects/8)
...
| `configuration.ComponentRegistry` typo fix | The second `registerDecorator` overload's javadoc said "a lowe order" — the near-identical overload just above it in the same file correctly says "a lower order." Copy-paste typo, fixed to match |
| `configuration.DefaultComponentRegistry` — two real gaps | `LocalConfiguration.fromParent`'s two overloads (`Class<C>` and `TypeReference<C>`) declared `name` non-null, but both are called from `getOptionalComponent`'s own `@Nullable String name` and forward straight into `Configuration.getOptionalComponent(..., @Nullable String)`. Added `@Nullable` to both. Separately, `getModuleConfiguration`'s `Assert.nonEmpty(name, "The name must not be null.")` only described half of what `nonEmpty` actually checks — `DecoratorDefinition.java`'s identical call already said "must not be empty or null," matched that |
| `configuration.BaseModule` message-polarity bug | `componentRegistry()`'s `requireNonNull(registryAction, "The registryAction must be null.")` had the message backwards — it fires when the value IS null, but the message claimed the opposite requirement. Fixed to "must not be null" |
| `configuration.DefaultAxonApplication` — three real gaps | A genuine upstream bug: an unused `org.jspecify.annotations.NonNull` import, confirmed never referenced anywhere in the file (only the unrelated `Objects.requireNonNull` static import shares the substring) — removed. `AxonConfigurationImpl.getParent()` returns `null` for the root configuration but had no `@Nullable`, while `Configuration.getParent()` is declared `@Nullable` and `DefaultComponentRegistry`'s own `LocalConfiguration.getParent()` already implements it correctly — matched. `registerLifecycleHandler`'s "configuration already initialized" check threw `IllegalArgumentException`, but nothing about an argument is wrong — it's a state/timing violation; `BaseModule`'s identical "already been built" check correctly uses `IllegalStateException` — matched |
| `.settings/org.eclipse.jdt.core.prefs` added | Fixed a real jdt.ls note ("At least one of the problems in category 'unused' is not analysed due to a compiler option being ignored," tied to `BaseModule`'s `@SuppressWarnings("unused")`) by explicitly configuring the `unused*` compiler-problem categories and pinning compliance/source/target to 25 to match the real Gradle toolchain. Carved a targeted exception into the blanket `.settings/` gitignore rule for this one file, since unlike `.classpath`/`.project` it's hand-authored and actually read by VS Code's Java extension |
| JDT Language Server heap increase | Recurring "Java Language Server encountered an OutOfMemory error" — default jdt.ls heap was too small for this project's size combined with `java.import.gradle.enabled` doing its own Gradle sync. Added `java.jdt.ls.vmargs` (`-Xmx3G` on this 16GB machine) to `.vscode/settings.json` and the workspace file |
| `.vscode/extensions.json` added, then synced to real disabled state | Curated recommendations for the actual Java/Gradle/Kotlin/Checkstyle/Docker/GitHub-Actions toolchain this project uses, flagging `vscode-maven` (wrong tool for a Maven→Gradle migration) and `even-better-toml` (no `.toml` files remain) as not relevant. Later read the real per-workspace disabled-extension list directly from VS Code's own storage and synced 120 genuinely-irrelevant ones into `unwantedRecommendations`, replacing the earlier hand-picked guess — found 6 recommended extensions (including `vscjava.vscode-gradle`) had been disabled too, almost certainly by accident during a bulk multi-select; flagged those separately for manual re-enabling rather than silently reconciling the contradiction |
| **`common` module complete — 144 of 144 files (2026-09-07)** | See the `common` module row above. First fully-completed reactor module beyond `test-logging`. Real-gap ledger count bumped 18 → 24 (six new entries this leg: the `ComponentRegistry` typo is cosmetic and not counted, matching the "fur fluent API" precedent) |
| DRY fix: per-module Maven scope-mapping comment centralized | Once a second module (`update`) needed the same "Maven `<scope>`/`optional` → Gradle configuration" explanation `common/build.gradle.kts` already carried, the comment was either going to be repeated a third, fourth, fifth time as more modules land, or cross-reference an arbitrary sibling module (`update`'s original comment pointed at `common/build.gradle.kts` specifically). Moved the full explanation to `axonframework.java-conventions.gradle.kts` instead — the one file every module's `build.gradle.kts` actually applies, making it the natural single source of truth rather than an arbitrary peer. Both `common` and `update` now just point to it |
| `scripts/wait-for-ci.sh` added | Every file-conversion commit during the migration cadence was checked against real CI status with the same hand-typed `until [gh run list ...]; do sleep 5; done` shell incantation, retyped from scratch each time rather than written once. Extracted into a small reusable script (`./scripts/wait-for-ci.sh <workflow-file>`), matching the existing `scripts/oracle-test-up.sh` convention. Needed the same executable-bit fix as `gradlew` originally did — Windows staged it `100644` by default, fixed with `git update-index --chmod=+x` |
| **SOLID redesign of `ComponentRegistry`/`DefaultComponentRegistry` (2026-09-07) — first real modernization, not just a conversion** | A SOLID audit (the same real-evidence rigor as the DRY check above) found `DefaultComponentRegistry` bundling five responsibilities (component storage, decorator application, enhancer scanning/invocation, module building, and a `Configuration`-lookup inner class) and `ComponentRegistry`'s interface being wider than its real usage needs. A tree-wide usage survey of the **entire real upstream Axon 5 source** (all modules, not just `common`) confirmed this was safe to fix: 8 of `ComponentRegistry`'s 10 abstract methods have heavy real callers across `messaging`/`eventsourcing`/`modelling`/`extensions` (up to 424 call sites) and were left untouched; the other 2 (`registerFactory`, `setOverridePolicy`) have **zero real call sites anywhere outside `common` itself**, so they were safely split into a new `ConfigurableComponentRegistry` interface. Added three new internal collaborator classes — `Modules`, `DecoratorDefinitions`, `ConfigurationEnhancers` — that `DefaultComponentRegistry` now delegates to, matching the existing `Components`/`ConfigurationExtensions` pattern already used in this package. Zero public API change from the SRP half; the ISP half changes nothing for any currently-existing Gradle-side code (zero test files exist anywhere in this fork yet). Planned via `EnterPlanMode` (real Explore-agent usage survey → Plan-agent design validation → user approval) rather than done ad hoc, given the architectural stakes. Real-gap ledger unaffected — this is a redesign, not a bug fix, tracked separately |
| Full comprehensive sweep (2026-09-07): retroactive DRY/SOLID re-audit of all 149 `common` files (144 converted + 5 net-new from the SOLID redesign) + doc-drift check | With `common` freshly complete, re-audited every file against the DRY/SOLID charter retroactively rather than only forward, using two independent research agents. Found and fixed 22 real issues, each individually verified (fresh read, diff against Maven source, real `compileJava`/`checkstyleMain`) and committed separately: **real contract bugs** — `CollectionUtils.intersect`'s javadoc claimed result order followed `collection2` when the implementation actually follows `collection1`; `Components.postProcessComponents`'s `requireNonNull` message had the same polarity bug as the earlier `BaseModule` one (said "must be null", meant "must not"); `FutureUtils.joinAndUnwrap(future, timeout)` was missing `@Nullable` despite its own javadoc documenting a null return, while the 1-arg overload that just delegates to it had the annotation; `IOUtils.closeQuietly`/`closeQuietlyIfCloseable` and `JdbcUtils.closeQuietly(ResultSet\|Statement\|Connection)` all had javadoc already documenting a nullable argument but no `@Nullable` on the parameter; `MavenArtifactVersionResolver.get()` returns `null` from two different paths without declaring it; `JdbcUtils`'s 4-arg `nextAndExtract`/`extract` overloads are called with a literal `null` `defaultValue` by their own 3-arg siblings but didn't declare it `@Nullable`; all four `Cache` implementations (`WeakReferenceCache`, `JCacheAdapter`, `EhCacheAdapter`, `NoCache`) override a `@Nullable`-declared interface method without repeating the annotation. **DRY fixes** — extracted a shared `DescribableComponent.resolvedTypeName()` static helper for a 3-line ternary duplicated identically in `FilesystemStyleComponentDescriptor` and `JacksonComponentDescriptor`; extracted `Components.getComponentsMatching()` from two near-identical private methods that only differed in which `Identifier` predicate they filtered by; `JdbcSQLErrorCodesResolver` reimplemented the exact cause-chain walk `ExceptionUtils.findException` already provides, now calls it directly; `PriorityCallable`/`PriorityRunnable` extracted their duplicated `equals`/`hashCode`/`toString`/priority/sequence fields into a new package-private `AbstractPriorityTask` base class. **Formatting** — replaced 8 remaining IntelliJ-only `//noinspection unchecked` comments with portable `@SuppressWarnings` across the `caching` package; squeezed 16 doubled blank lines (mostly a stray line left behind when `@Nullable` imports were inserted between existing import groups) across 12 files; converted `DirectlyAccessedProperty`'s tab indentation to this project's 4-space convention; fixed a malformed `{@link Callable<T>}` javadoc tag, a doubled space in `ThrowingFunction` and `ClockUtils`, a misordered `private @Nullable static` modifier sequence in `AnnotationUtils`, misaligned continuation-line indentation in two `ProcessUtils` overloads, and a pointless `Optional.of(...).get()` wrapper in `BaseModule.build()`. Real-gap ledger count bumped 24 → 31 (the 7 real contract-bug findings above; DRY extractions and formatting fixes aren't gap-ledger material, matching the `ComponentRegistry` typo precedent). Commit count corrected 227 → 236, days 16 → 17 |
| Dependabot security sweep (2026-09-09): 7 of 7 open alerts fixed | GitHub's dependency graph flagged 7 open alerts, all attributed to `settings.gradle.kts` (this build's own tooling classpath, not anything published) — none were declared anywhere in this repo's own build files, so each needed tracing to its real transitive source before it could be fixed. **AssertJ** (fixed in 3.27.7): already resolved — `axonframework.java-conventions.gradle.kts` already declared `testImplementation("org.assertj:assertj-core:3.27.7")`; the alert was just lagging behind GitHub's next re-scan, confirmed by checking the real resolved version in every module. **Commons Lang3 and Plexus-Utils** (fixed in 3.18.0 / 3.6.1): Checkstyle 13.10.0 transitively pulls in Maven's doxia reporting modules (used only for a site-report format this build never invokes) which resolve both at vulnerable versions — forced to patched versions via `resolutionStrategy` scoped to the `checkstyle` configuration, verified Checkstyle itself still runs clean afterward. **Plexus-Utils had a second, separate source**, missed on the first pass and caught only because the alert stayed open after the first fix and pushing further paid off: `test-logging`'s own published `log4j-core-test` dependency pulls in `maven-core` → `maven-model` → plexus-utils 3.6.0, one patch version short of the fix — needed forcing project-wide (every configuration, not just `checkstyle`) since `test-logging` is api-exposed and every module depends on it in test scope. **Three Bouncy Castle alerts** (bcpg/bcprov/bcpkix, fixed in 1.84): traced to Gradle's own bundled `signing` plugin (`useInMemoryPgpKeys` in `published-conventions.gradle.kts`) shipping its own embedded Bouncy Castle — not something forceable from a project's own `build.gradle.kts` at all. Fixed by bumping the Gradle wrapper 9.2.0 → 9.7.1, verified empirically (extracted the actual distribution's `lib/plugins` directory, not just changelog reading) to bundle Bouncy Castle 1.85 and to no longer ship `bcpkix-jdk18on` at all (replaced by `bcutil-jdk18on`). **The seventh (JetBrains Kotlin Gradle Plugin, CVE-2026-53914) initially had no available fix** — needs `kotlin-gradle-plugin` >= 2.4.20, but the `kotlin-dsl` plugin `build-logic` applied bundles whatever version Gradle's own release ships (2.4.0 in 9.7.1, still only 2.4.10 in the 9.8.0 release candidate). A direct force attempt in `build-logic/build.gradle.kts`'s script body was tested and confirmed to have zero effect (plugin-classpath resolution for `plugins { }` DSL usage happens before the script body runs) and was reverted rather than left as dead code. **Actually resolved the same day** — see the row directly below for how |
| `build-logic`'s 3 convention plugins rewritten in Java, eliminating the Kotlin Gradle Plugin CVE outright (2026-09-09) | This project's build never enables Gradle's build cache anywhere (no `buildCache {}` block, no `org.gradle.caching=true`, no `--build-cache` flag in CI), so the CVE's actual exploitable path (deserializing build-cache metadata) was never reachable — offered to the user as grounds to just dismiss the GitHub alert with that documented reason, but the user chose to eliminate the dependency for real instead. Rather than keep Kotlin and pin a newer, unofficially-paired compiler version (the first proposal), the three precompiled Kotlin-script convention plugins (`axonframework.java-conventions`/`published-conventions`/`internal-conventions`) were rewritten as plain Java classes implementing `Plugin<Project>`, registered via `gradlePlugin { plugins { create(...) } }` instead of `kotlin-dsl`'s automatic precompiled-script-plugin discovery. This removes Kotlin from `build-logic` entirely, not just to a newer version — confirmed via `buildEnvironment` showing zero Kotlin references afterward, where before the change `kotlin-gradle-plugin` was clearly present. New package is `com.terrencedaniels.buildlogic`, not `org.axonframework.buildlogic` — that namespace is reserved elsewhere in this fork for published, upstream-namespace-compatible library code, a distinction `published-conventions` itself already states explicitly; `build-logic` is never published and has no such rationale. Planned via `EnterPlanMode` given the stakes (rewriting the mechanism every module in this build depends on to even apply its own conventions) — a Plan-agent review `javap`'d the real Gradle 9.7.1 API classes rather than trusting assumptions, which mattered: Kotlin's `foo = bar` script syntax is ambiguous between a plain setter and a lazy `Property<T>.set()` call, and `MavenPublication`'s own identity fields turned out to be plain setters while `MavenPom` one level down, in the same `publishing {}` block, is entirely `Property`-based — exactly the kind of inconsistency a line-by-line translation could get wrong without checking each one individually. One real mistranslation was caught this way before committing: the `test-logging` dependency line initially used `Project.project(String)` instead of `DependencyHandler.project(String)`, which surfaced as a genuine Gradle-10-incompatibility deprecation warning, not a cosmetic one. Verified exhaustively before committing — `buildEnvironment`, the checkstyle and `testRuntimeClasspath` dependency trees, the generated POM, the jar manifest, the test-jar contents, the checkstyle report, and the full task inventory for `:common` were all confirmed byte-for-byte identical before and after; `update`'s real, currently-exercised dependency on `common`'s `testArtifacts` configuration was confirmed to still resolve correctly. The one path ordinary CI never exercises (the conditional signing block) couldn't be run end-to-end in this environment (`gpg-agent` wouldn't start for either RSA or ed25519 key generation), but both `useInMemoryPgpKeys(String, String)` and `sign(Publication...)` were confirmed against the real API via `javap`, and neither line in that block is a property assignment, so there was no setter-vs-`Property` ambiguity for that specific code path to begin with. `build-logic`'s own toolchain stays at JDK 21 (unrelated, already-completed fix) — every real module's published artifacts are unaffected and still require JDK 25, confirmed by identical bytecode major version in `common`'s compiled classes before and after. All 7 of the original 7 Dependabot alerts are now closed |

**Still to do:** the rest of the Maven reactor — `common` is now fully done (144/144). What's left: `build/parent`'s
remaining publications, `axon-framework-bom`, `conversion`, `extensions` (its own sub-reactor: `kotlin`, `metrics`,
`reactor`, `spring`), `eventsourcing`, `messaging`, `migration`, `modelling`, `test`, `update`, `integrationtests`,
`docs/_samples` —
